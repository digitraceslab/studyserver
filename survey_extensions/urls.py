from django.urls import re_path

from survey_extensions.views import ExtendedSurveyDetail, survey_csv_disabled

# Anchored (fully matching) patterns so that only the survey-detail routes are
# taken over here; every other survey.urls route (list/completed/confirmation)
# falls through to the library's urls.py, mounted right after this one.
# The csv route is shadowed on purpose: the library's export leaks usernames.
urlpatterns = [
    re_path(r"^(?P<id>\d+)/$", ExtendedSurveyDetail.as_view(), name="survey-detail"),
    re_path(r"^(?P<id>\d+)-(?P<step>\d+)/$", ExtendedSurveyDetail.as_view(), name="survey-detail-step"),
    re_path(r"^csv/(?P<primary_key>\d+)/$", survey_csv_disabled, name="survey-result"),
]
