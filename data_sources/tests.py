...
from data_sources.models import db_connector, portability_client
from studies.models import Consent, Study, StudySourceConfiguration
...
class DbConnectorTest(TestCase):
    ...
    def test_query_aware_data_returns_transformed_rows(self):
                ...
                if "select id, device_uuid from device_lookup" in qstr:
                    ...
                    if params and self._rows:
                        ...
                        if self.dictionary:
                            self._rows = [
                                r
                                for r in self._rows
                                if str(r.get("device_uuid")) in set(map(str, vals))
                            ]
                        else:
                            self._rows = [
                                tuple([r.get("id"), r.get("device_uuid")])
                                for r in self._rows
                                if str(r.get("device_uuid")) in set(map(str, vals))
                            ]
                    else:
                        self._rows = (
                            [{"id": 42, "device_uuid": "dev-uuid"}]
                            if self.dictionary
                            else [(42, "dev-uuid")]
                        )
                    ...
                    if src_rows:
                        ...
                        if self.dictionary:
                            ...
                            self._rows = [
                                (r.get("device_uid"), r.get("val")) for r in filtered
                            ]
                    else:
                        if self.dictionary:
                    ...
                    "select" in qstr
                    and "_transformed" not in qstr
                    and "device_lookup" not in qstr
                    and "show tables" not in qstr
                ):
        ...
        class FakeConnection:
            def __init__(self):
                ...
                self._first = FakeCursor(
                    rows=[("battery",), ("battery_transformed",), ("other_table",)]
                )
                # device_lookup mapping and other dictionary cursor results
                self._second = FakeCursor(
                    rows=[
                        {"id": 42, "device_uuid": "dev-uuid"},
                        {"id": 99, "device_uuid": "other-uuid"},
                    ],
                    dictionary=True,
                )
        ...
        with patch(
            ...
            with patch(
                "data_sources.models.db_connector.get_device_ids_for_label",
                return_value=["dev-uuid"],
            ):
                # Query for device 'dev-uuid' should return only transformed rows
                rows = db_connector.query_aware_data(
                    "SELECT *", "label-1", "battery", limit=10
                )
                ...
                with patch(
                    ...
                    rows2 = db_connector.query_aware_data(
                        "SELECT *", "label-1", "battery", limit=10
                    )
                ...
                with patch(
                    "data_sources.models.db_connector.query_aware_data",
                    return_value=[{"row_count": 5}],
                ):
    ...
    def test_auto_create_data_source_with_only_name_field(self):
        ...
        self.assertTrue(
            AwareDataSource.objects.filter(
                profile=self.profile, name__istartswith="aware"
            ).exists()
        )
    ...
    def test_create_data_source_via_post(self):
        response = self.client.post(
            reverse("add_data_source", args=["aware"]), {"name": "My New Source"}
        )
        self.assertEqual(response.status_code, 302)
        source = AwareDataSource.objects.filter(profile=self.profile).latest("id")
        expected_url = reverse("instructions", args=[source.id])
        ...
        self.assertTrue(
            AwareDataSource.objects.filter(
                profile=self.profile, name="My New Source"
            ).exists()
        )
        self.assertEqual(source.name, "My New Source")

    def test_create_json_data_source_with_extra_field(self):
        post_data = {"name": "My JSON Source", "url": "https://example.com/data.json"}
        response = self.client.post(
            reverse("add_data_source", args=["json_url"]), post_data
        )
        self.assertEqual(response.status_code, 302)

        source = JsonUrlDataSource.objects.filter(
            profile=self.profile, name="My JSON Source"
        ).latest("id")
...
class DataSourcesViewsTest(TestCase):
    def setUp(self):
        ...
        self.source = AwareDataSource.objects.create(
            profile=self.profile, name="Test Aware Source"
        )
    ...
    def test_user_cannot_delete_others_source(self):
        ...
        response = self.client.post(
            reverse("delete_data_source", args=[self.source.id])
        )
        self.assertEqual(response.status_code, 404)
...
class AwareDataSourceTest(TestCase):
    ...
    def test_device_id_conflict_raises_validation_error(self):
        ...
        src1 = AwareDataSource.objects.create(
            profile=self.profile, name="S1", device_id=shared_id
        )
        src2 = AwareDataSource(profile=profile2, name="S2", device_id=shared_id)
        with self.assertRaises(ValidationError):
            src2.save()

    def test_supports_deletion_returns_true(self):
        self.assertTrue(self.source.supports_deletion())

    def test_mark_deletable_calls_insert_with_correct_params(self):
        # Mock the db_connector.insert_deletion_request function
        with patch(
            "data_sources.models.db_connector.insert_deletion_request"
        ) as mock_insert:
            # Call mark_deletable with through=1000
            result = self.source.mark_deletable("battery", through=1000)

            # Verify it was called with delete_before=1001 (through + 1)
            mock_insert.assert_called_once_with(
                self.source.device_label, "battery", 1001
            )

            # Verify it returns the result from the mock
            self.assertEqual(result, mock_insert.return_value)

    def test_mark_deletable_with_string_through(self):
        # Test that string through values are converted to int
        with patch(
            "data_sources.models.db_connector.insert_deletion_request"
        ) as mock_insert:
            result = self.source.mark_deletable("battery", through="1000")
...
class JsonUrlDataSourceTest(TestCase):
    ...
    def test_fetch_enriches_and_count(self, mock_get):
        ...
        study = Study.objects.create(
            title="S", description="d", config_url="http://example.com"
        )
        ...
        mock_resp.json.return_value = [
            {"foo": "bar", "device_id": "old-id", "timestamp": 1000}
        ]
...
class PortabilityModelTestMixin:
    ...
    @patch(
        "data_sources.models.portability_client.get_data",
    ...
    @patch(
        "data_sources.models.portability_client.get_data", side_effect=Exception("boom")
    )
    ...
    @patch(
        "data_sources.models.portability_client.delete_donation",
    ...
    @patch("data_sources.models.portability_client.get_donation")
    def test_process_data_authorized_sets_authorized_processing_status(
        self, mock_get_donation
    ):
    ...
    @patch("data_sources.models.portability_client.get_donation")
    def test_process_data_without_donation_id_returns_without_api_call(
        self, mock_get_donation
    ):
        source = self._make_source(donation_id=None)
        source._process_data()
    ...
    @patch(
        "data_sources.models.portability_client.get_donation",
...
class PortabilityViewCreationRollbackTest(TestCase):
    ...
    @patch(
        "data_sources.models.portability_client.create_donation",
...
class PortabilityViewDeleteTest(TestCase):
    ...
    def test_delete_niimport_source_removes_from_db(self, _mock):
        ...
        self.client.post(reverse("delete_data_source", args=[source.id]))
        self.assertFalse(NiimportDataSource.objects.filter(id=source.id).exists())
...
class NiimportDisplayTypeForConfigurationTest(TestCase):
    ...
    def test_tiktok_export(self):
        self.assertEqual(
            NiimportDataSource.display_type_for_configuration(
                {"niimport_source_type": "tiktok_export"}
            ),
            "TikTok Export Data",
        )
...
class AddDataSourceCopiesConsentParamsTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="consent_copy_user", password="testpass"
        )
        self.profile = Profile.objects.create(user=self.user)
        self.client.login(username="consent_copy_user", password="testpass")
        self.study = Study.objects.create(
            title="Consent Copy Study", config_url="http://example.com"
        )
...
class DeletableWatermarkTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="watermark_user", password="testpass"
        )
        self.profile = Profile.objects.create(user=self.user)
        self.source = AwareDataSource.objects.create(
            profile=self.profile,
    ...
    def test_same_data_type_different_sources_allowed(self):
        ...
        user2 = User.objects.create_user(
            username="watermark_user2", password="testpass"
        )
        profile2 = Profile.objects.create(user=user2)
        source2 = AwareDataSource.objects.create(profile=profile2, name="Second Source")
        DeletableWatermark.objects.create(
            data_source=self.source, data_type="battery_transformed"
        )
        wm2 = DeletableWatermark.objects.create(
            data_source=source2, data_type="battery_transformed"
        )
...
class AwareConfigEndpointTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="aware_cfg_user", password="testpass"
        )
        self.profile = Profile.objects.create(user=self.user)
        self.study = Study.objects.create(
            title="Aware Config Study",
            description="desc",
